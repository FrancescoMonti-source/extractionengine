...
| Record LLM annidato in `values` | **Rifiutato per ora** | `values` resta piatto e direttamente analizzabile. Il record annidato eliminerebbe le collisioni con i nomi del motore, ma introdurrebbe list-column, accesso meno immediato e una rottura dell'API senza un caso reale che la richieda. I nomi riservati restano validati fail-fast; si riapre davanti a una collisione concreta. |
